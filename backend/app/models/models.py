...
class User(Base):
    ...
    saron_reference_annual_pct: Mapped[float] = mapped_column(
    ...
    pension_data: Mapped[List["PensionData"]] = relationship(
        "PensionData", back_populates="user", cascade="all, delete-orphan"
    )
    ...
    import_logs: Mapped[List["ImportLog"]] = relationship(
        "ImportLog", back_populates="user", cascade="all, delete-orphan"
    )
    forecast_scenarios: Mapped[List["ForecastScenario"]] = relationship(
        "ForecastScenario", back_populates="user", cascade="all, delete-orphan"
    )
...
class Account(Base):
    ...
    import_logs: Mapped[List["ImportLog"]] = relationship(
        "ImportLog", back_populates="account"
    )
...
class Budget(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "user_id", "category_id", "period", "year", "month", name="uq_budget"
        ),
    )
...
class PensionData(Base):
    ...
    average_insured_salary: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True
    )
    notes: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
...
class Asset(Base):
    ...
    asset_type: Mapped[AssetType] = mapped_column(
        Enum(AssetType, native_enum=False), nullable=False
    )
    name: Mapped[str] = mapped_column(String(255), nullable=False)
...
class MortgageTranche(Base):
    ...
    mortgage_type: Mapped[str] = mapped_column(
        String(16), nullable=False, default="fix"
    )
...
class ProjectionCache(Base):
    ...
    scenario: Mapped[Optional["Scenario"]] = relationship(
        "Scenario", back_populates="projections"
    )
...
class ImportLog(Base):
    ...
    account: Mapped[Optional["Account"]] = relationship(
        "Account", back_populates="import_logs"
    )
...
class PeerGroupBenchmark(Base):
    ...
    household_type: Mapped[str] = mapped_column(
        String(30), nullable=False, default="single"
    )
    ...
    communication_avg: Mapped[float] = mapped_column(Float, default=0.0)
    clothing_avg: Mapped[float] = mapped_column(Float, default=0.0)
    restaurant_avg: Mapped[float] = mapped_column(Float, default=0.0)
